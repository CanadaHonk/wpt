...
function very_long_frame_duration() {
  return 360;
}
...
async function expect_long_frame(cb, t) {
  ...
  const timeout = new Promise((resolve, reject) =>
    t.step_timeout(() => reject("timeout"), very_long_frame_duration() * 3));
...
async function expect_no_long_frame(cb, t) {
  ...
  for (let i = 0; i < 5; ++i) {
    ...
    const result = await Promise.race([receivedLongFrame,
        new Promise(resolve => t.step_timeout(() => resolve("timeout"), very_long_frame_duration() * 2))]);
    if (result === "timeout")
      return false;
  }
